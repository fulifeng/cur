...
def mcsir_full_cv():
    print '-------------Cross Validation MCSIR--------------'
    os.chdir(working_dir)
    clusters = 1
    cluster_fnames_fname = './laplacian_matrices/clusters_' + str(clusters) + '.csv'
    # model_paras = {'lam_i': 100.0, 'thres': 0.004, 'lam_m': 2.0, 'lam_r': 7.0e5}
    # model_paras = {'lam_i': 80.0, 'thres': 0.004, 'lam_m': 5.0, 'lam_r': 100000.0}
    # # mcsir-cc
    # clusters = 1
    # cluster_fnames_fname = './laplacian_matrices/clusters_' + str(clusters) + '.csv'
    # model_paras = {'lam_i': 80.0, 'thres': 0.004, 'lam_m': 5.0, 'lam_r': 100000.0}
    ...
    clusters = 3
    cluster_fnames_fname = './laplacian_matrices/clusters_' + str(clusters) + '.csv'
    model_paras = {'lam_i': 100.0, 'thres': 0.004, 'lam_m': 0.8, 'lam_r': 100000.0}
    best_para_list = []
    for i in range(folds):
        # model_paras['thres'] *= 2
        best_para_list.append(copy.copy(model_paras))
    model = mcsir.mcsir(model_paras, cluster_fnames_fname, final_list_fname, historical_ranking_list, confidence_histo_rank_list, hr_scale=2)
    evaluator = feval.fold_evaluator(gt_fname)
    tune = True
    tune = False
    if tune:
        # validator = cv.cross_validation(model, gt_fname, folds, test=True, tune=False)
        selected_paras = ['lam_i', 'lam_m', 'lam_r']
        para_bounds = [[1e2, 1e3], [1e-1, 1e1], [1e5, 1e6]]
        best_per = grid_search_cv(model, evaluator, model_paras, selected_paras, para_bounds)
...
def mcsir_full():
    ...
    clusters = 3
    cluster_fnames_fname = './laplacian_matrices/clusters_' + str(clusters) + '.csv'
    model_paras = {'lam_i': 10.0, 'thres': 0.004, 'lam_m': 0.1, 'lam_r': 200000.0}
    model = mcsir.mcsir(model_paras, cluster_fnames_fname, final_list_fname, historical_ranking_list, confidence_histo_rank_list, hr_scale=2)
    evaluator = feval.fold_evaluator(gt_fname)
    tune = True
    tune = False

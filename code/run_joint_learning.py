...
def multi_modal_cv():
    ...
    model_paras = {'lam_i': 400.0, 'lam_r': 100.0, 'thres': 0.004}
    model = multi_modal.multi_modal(model_paras, feature_fname_fname, final_list_fname, historical_ranking_list, hr_scale=2)
    ...
    if tune:
        ...
        # selected_paras = ['lam_i', 'thres', 'lam_r']
        # para_bounds = [[1e1, 1e3], [1e-4, 1e-2], [1e2, 1e4]]
        selected_paras = ['lam_i', 'lam_r']
        para_bounds = [[1e1, 1e3], [1e2, 1e4]]
...
def multi_modal_full():
    print '---------Full Rank Multi Modal----------'
    os.chdir(working_dir)
    model_paras = {'lam_i': 300.0, 'lam_r': 1e0, 'thres': 0.001}
    model = multi_modal.multi_modal(model_paras, feature_fname_fname, final_list_fname, historical_ranking_list, hr_scale=2)
    evaluator = feval.fold_evaluator(gt_fname)
    tune = True
    # tune = False
